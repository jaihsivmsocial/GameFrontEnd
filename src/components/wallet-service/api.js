...
export const walletAPI = {
  ...
  getActiveQuestion: async () => {
    try {
      console.log("Fetching active question from API endpoint:", `${API_BASE_URL}/bets/active-question`)
      ...
      // Log the exact question data received from the API
      if (data.success && data.question) {
        console.log("API getActiveQuestion received raw question data:", data.question)
        console.log("API getActiveQuestion exact question text:", data.question.question)

        // Store the exact question in localStorage for persistence
        if (typeof window !== "undefined") {
          try {
            localStorage.setItem("currentBetQuestion", JSON.stringify(data.question))
            console.log("Stored question in localStorage:", data.question)
          } catch (e) {
            console.error("Error storing question in localStorage:", e)
          }
        }

        // Dispatch an event with the question data
        if (typeof window !== "undefined") {
          window.dispatchEvent(
            new CustomEvent("active_question_loaded", {
              detail: { question: data.question },
            }),
          )
        }
      } else {
        console.log("API getActiveQuestion: No question data or success=false", data)
      }
...
export const bettingAPI = {
  ...
  getActiveQuestion: async () => {
    try {
      // First check if we can get the question from the socket
      const socket = getSocket()
      if (socket && socket.connected) {
        // Request the question via socket first
      ...
      // If we got a question, dispatch an event so other components can update
      if (data.success && data.question) {
        console.log("API returned question:", data.question)
        console.log("EXACT API QUESTION TEXT:", data.question.question)
  ...
  // Add a new function to create a betting question
  createBetQuestion: async (streamId = null) => {
    try {
      // First try to create via socket for immediate response
      const socket = getSocket()
      if (socket && socket.connected) {
      ...
      // Then make the API call as a backup
      const token = getAuthToken()
      const currentStreamId = streamId || getStreamId()

      const response = await fetch(`${API_BASE_URL}/bets/create-question`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
      ...
      // If we got a new question, dispatch an event
      if (data.success && data.question) {
        console.log("API created question:", data.question)
        console.log("EXACT API CREATED QUESTION TEXT:", data.question.question)
  ...
  // Force refresh question
  forceRefreshQuestion: async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/bets/refresh-question`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${getAuthToken()}`,
      ...
      // Dispatch a custom event for the refreshed question
      if (data.success && data.question) {
        if (typeof window !== "undefined") {
          window.dispatchEvent(
            new CustomEvent("question_refreshed", {
              detail: { question: data.question },
            }),
          )
        }
      ...
      console.error("Error forcing question refresh:", error)
      return { success: false, error: error.message }
    }
  },
}

// Add a helper function to get the stream ID
const getStreamId = () => {
