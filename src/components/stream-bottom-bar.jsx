...
// console.log(amountNeeded,"amount needed")
...
import { socketEvents, initializeSocket, getSocket, getCurrentCameraHolder } from "../components/wallet-service/socketService"
import { walletAPI } from "../components/wallet-service/api"
...
useEffect(() => {
  ...
  const handleCameraHolderUpdate = (event) => {
    console.log("Camera holder update event received:", event.detail)
  ...
  // Listen for camera holder updates from socket
  socket.on("camera_holder_update", (data) => {
    console.log("Camera holder update received from socket:", data)
  ...
  try {
    ...
    if (response.success && response.question) {
      setCurrentQuestion({
        id: response.question._id || response.question.id,
        question: response.question.question,
        subject: response.question.subject || cameraHolder?.CameraHolderName || "Player",
        condition: response.question.condition || "complete the challenge",
        endTime: new Date(response.question.endTime),
        yesPercentage: response.question.yesPercentage || 50,
        noPercentage: response.question.noPercentage || 50,
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <span style={{ fontSize: "16px", color: "white" }}>Will </span>
          <span style={{ color: "#06b6d4" }}>{cameraHolder?.CameraHolderName || currentQuestion?.subject || "James5423"}</span>
          <span style={{ fontSize: "16px", color: "white" }}>
            {" "}
            {currentQuestion?.condition || "survive the full 5 minutes"}?
          </span>
