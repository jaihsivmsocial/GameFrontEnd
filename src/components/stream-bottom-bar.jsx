...
export default function StreamBottomBar() {
  ...
  const handleNewQuestion = (data) => {
    ...
    // Use the question data directly from the backend
    const normalizedQuestion = {
      id: data.id || data._id,
      question: data.question, // Use the exact question from backend
      ...
      if (event.detail && event.detail.cameraHolder) {
        ...
        if (event.detail.cameraHolder.CameraHolderName && event.detail.cameraHolder.CameraHolderName !== "None") {
          ...
          setTimeout(() => {
            ...
            // Also force create a new question
            const socket = getSocket()
            if (socket && socket.connected) {
              console.log("Forcing question creation on camera holder update")
              socket.emit("create_bet_question", { streamId: getStreamId() })
            }
          }, 0)
        }
