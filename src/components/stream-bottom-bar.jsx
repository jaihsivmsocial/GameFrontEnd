...
export default function StreamBottomBar() {
  ...
  const calculatePotentialPayout = (amount, choice) => {
    ...
    // Calculate potential payout: bet amount + (bet amount * odds * (1 - platform fee))
    const potentialWinnings = betAmount * odds * (1 - platformFee)
    const totalPayout = betAmount + potentialWinnings
    ...
    socketEvents.onBetPlaced((data) => {
      ...
      if (data.questionId === currentQuestion?.id) {
        ...
        if (data.totalBetAmount !== undefined) {
          setTotalBets(formatCurrency(data.totalBetAmount))
        }
        ...
        if (data.totalPlayers !== undefined) {
          ...
          setTotalPlayers((prevCount) => prevCount + 1)
        }
  ...
  const fetchBettingStats = async () => {
    try {
      ...
      if (response.success && response.stats) {
        ...
        // Set mock data for testing if no real data is available
        setTotalBets("$5000")
        setBiggestWin("$1200")
        setTotalPlayers(42)
      }
      ...
      // Set mock data for testing if there's an error
      setTotalBets("$5000")
      setBiggestWin("$1200")
      setTotalPlayers(42)
    }
  ...
  const updateStatsAfterBet = (betAmount) => {
    ...
    // Update total bets by adding the new bet amount
    const currentTotalBetsStr = totalBets.startsWith("$") ? totalBets.substring(1).replace(/,/g, "") : "0"
    const currentTotalBets = Number.parseFloat(currentTotalBetsStr) || 0
    const newTotalBets = currentTotalBets + betAmount

    console.log("Current total bets:", currentTotalBets, "New total bets:", newTotalBets)
    setTotalBets(formatCurrency(newTotalBets))

    // Increment player count
    setTotalPlayers((prevCount) => {
      console.log("Incrementing player count from", prevCount, "to", prevCount + 1)
      return prevCount + 1
    })
  }
        ...
        >
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <div>
              ...
              <span style={{ color: "#06b6d4" }}> {totalBets}</span>
            </div>
            <div>
          ...
          <div style={{ textAlign: "center" }}>
            <span style={{ color: "white", fontWeight: "bold" }}>{totalPlayers} Players</span> Have Already Bet
          </div>
            ...
            <div style={{ textAlign: "center" }}>
              ...
              >
                <div style={{ textAlign: "left" }}>
                  <div>
                    Total Bets: <span style={{ color: "#06b6d4" }}>{totalBets}</span>
                  </div>
                  <div>
