...
import { useMediaQuery } from "../../components/chat/use-mobile" // Keep this import
 import ChatBetContainer from "../chatbet/chat-bet-container" // Import ChatBetContainer
import StreamBottomBar from "../stream-bottom-bar copy"
...
const RealTimeChatComp = ({ streamId = "default-stream", isReadOnly = false }) => {
  ...
  const isMobile = useMediaQuery("(max-width: 768px)") // Keep this hook
 ...
 const [activeTab, setActiveTab] = useState('chat');
  ...
  // Desktop UI (original implementation with table structure)
  return (
    ...
    <>
      ...
      {!isReadOnly && (
        <form onSubmit={handleSendMessage} className={styles.chatInput}>
          ...
          <button type="submit" className={styles.sendButton} disabled={!isConnected || !message.trim()}>
            <img
              src="/assets/img/chat/send-message.png"
              width={20}
              height={20}
              alt="Send"
              className={styles.icon}
